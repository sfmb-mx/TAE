...
## problems discussed in this lecture can be solved in R using the
## "lpSolveAPI" package. This package allows you to solve linear
## optimization problems in R using the lp_solve library, which is an
## open-sourced optimization solver.
...
## are other optimization packages available in R that rely on
## commercial solvers, such as cplexAPI and Rcplex. For more
## information, see the CRAN Optimization page:
## http://cran.us.r-project.org/web/views/Optimization.html

## First, you need to install and load the lpSolveAPI package:

## install.packages('lpSolveAPI', repos='http://cran.rstudio.com/')
...
## Now we need to set up the constraints and objective. The best way
## to do this using the lpSolveAPI package is by viewing the
## constraints in a matrix format. Our objective and three constraints
## are as follows:
...
## are constrained to be non-negative, which is the default
## setting. If you want to change the lower or upper bounds for a
## decision variable, you can do so with the set.bounds
## function. (Next week we will learn about integer and binary
## decision variables - to change a decision variable to one of these
...
## An output of zero means that the model was successfully solved. You
## can look at the optimal objective function value or optimal
## decision variable values with get.objective and get.variables:

get.objective(AirlineSimple)
get.variables(AirlineSimple)

## You should get an objective value of 77408 and decision variable
## values of 100 and 66, just like we did in lecture.
...
## You should get an objective function value of 120514 and decision
## variable values of 80, 0, 75, 11, 60, 26.

## For additional examples, we direct you to the lpSolveAPI User's
## Guide:
