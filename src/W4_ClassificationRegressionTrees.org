...
In this recitation, Iain will be using the dataset [[https://courses.edx.org/asset-v1:MITx%2B15.071x_2a%2B2T2015%2Btype@asset%2Bblock/boston.csv][boston.csv]] to
predict housing prices in Boston. Please download this dataset to
follow along in this recitation. This data comes from the
[[http://archive.ics.uci.edu/ml/datasets/Housing][UCI Machine Learning Repository]].

An R script file with all of the R commands used in this recitation
can be downloaded [[https://courses.edx.org/asset-v1:MITx%2B15.071x_2a%2B2T2015%2Btype@asset%2Bblock/Unit4_Recitation.R][here]].
...
[19] "USDA.csv"                "WHO.csv"
[21] "WHO_Europe.csv"          "baseball.csv"
[23] "boston.csv"              "climate_change.csv"
[25] "framingham.csv"          "gerber.csv"
[27] "loans.csv"               "loans_imputed.csv"
[29] "mvtWeek1.csv"            "parole.csv"
[31] "pisa2009test.csv"        "pisa2009train.csv"
[33] "quality.csv"             "songs.csv"
[35] "stevens.csv"             "wine.csv"
[37] "wine_test.csv"
#+end_example
...
We are interested in *building a model* initially of *how prices vary* by
*location across a region*. So let's first see how the points are laid
out. Using the plot commands, we can plot the latitude and longitude
of each of our census tracts.

#+BEGIN_SRC R :var basename="PositionsCensusTracts" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION: Latitude and longitude of the census tracts.
#+NAME:    fig:PositionsCensusTracts
#+ATTR_LaTeX: placement: [H]
[[../graphs/PositionsCensusTracts.png]]
...
So we want to show all the points that lie along the *Charles River* in
a different color. We have a variable, *CHAS*, that tells us if a point
is on the Charles River or not.

#+BEGIN_SRC R :var basename="CHASRiverLocation" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Adding census tracts along Charles River
#+NAME:     fig:CHASRiverLocation
#+ATTR_LaTeX: placement: [H]
[[../graphs/CHASRiverLocation.png]]
...
#+BEGIN_SRC R :var basename="MITcensusLocation" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
  ...
  points(boston$LON[boston$CHAS == 1], boston$LAT[boston$CHAS == 1], col
         = "blue", pch = 19)

  ## Plot MIT
  points(boston$LON[boston$TRACT == 3531], boston$LAT[boston$TRACT ==
                                3531], col = "red", pch = 19)
...
#+CAPTION:  Plotting the MIT census tract
#+NAME:     fig:MITcensusLocation
#+ATTR_LaTeX: placement: [H]
[[../graphs/MITcensusLocation.png]]

*What other things can we do?* Well, this data set was originally
...
#+BEGIN_SRC R :var basename="PollutionPlotBoston" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Pollution in Boston city
#+NAME:     fig:PollutionPlotBoston
#+ATTR_LaTeX: placement: [H]
[[../graphs/PollutionPlotBoston.png]]
...
#+BEGIN_SRC R :var basename="BostonPricesLocations" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
  ...
  # Plot prices
  plot(boston$LON, boston$LAT)
  points(boston$LON[boston$MEDV >= 21.2], boston$LAT[boston$MEDV >=
                                          21.2], col = "red", pch = 20)
...
#+CAPTION:  Locations in Boston respect to prices
#+NAME:     fig:BostonPricesLocations
#+ATTR_LaTeX: placement: [H]
[[../graphs/BostonPricesLocations.png]]
...
#+BEGIN_SRC R :var basename="LatitudeVsPricePlot" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Latitude vs. Price plot.
#+NAME:     fig:LatitudeVsPricePlot
#+ATTR_LaTeX: placement: [H]
[[../graphs/LatitudeVsPricePlot.png]]

Now we can plot the Longitude vs. Price and see if we have any
relationship between these variables.

#+BEGIN_SRC R :var basename="LongitudeVsPricePlot" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Longitude vs. Price correlation
#+NAME:     fig:LongitudeVsPricePlot
#+ATTR_LaTeX: placement: [H]
[[../graphs/LongitudeVsPricePlot.png]]
...
#+BEGIN_SRC R :var basename="PositionCoordinatesVsPriceModel" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Position vs. Price in our linear model
#+NAME:     fig:PositionCoordinatesVsPriceModel
#+ATTR_LaTeX: placement: [H]
[[../graphs/PositionCoordinatesVsPriceModel.png]]
...
#+BEGIN_SRC R :var basename="CARTBostonLocationModel" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  CART plot of the Boston prices vs. position.
#+NAME:     fig:CARTBostonLocationModel
#+ATTR_LaTeX: placement: [H]
[[../graphs/CARTBostonLocationModel.png]]
...
#+BEGIN_SRC R :var basename="VisualizeOutput" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Visualizing the output.
#+NAME:     fig:VisualizeOutput
#+ATTR_LaTeX: placement: [H]
[[../graphs/VisualizeOutput.png]]
...
#+BEGIN_SRC R :var basename="PriceVsPositionWIncreasedMinbucket" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Increasing minbucket to simplify the CART.
#+NAME:     fig:PriceVsPositionWIncreasedMinbucket
#+ATTR_LaTeX: placement: [H]
[[../graphs/PriceVsPositionWIncreasedMinbucket.png]]

Or using the ~prp~ commmand:

#+BEGIN_SRC R :var basename="PriceVsPositionWIncreasedMinbucket02" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Same CART with increased minbucket and prp command.
#+NAME:     fig:PriceVsPositionWIncreasedMinbucket02
#+ATTR_LaTeX: placement: [H]
[[../graphs/PriceVsPositionWIncreasedMinbucket02.png]]
...
#+BEGIN_SRC R :var basename="CART2ModelBoston" :session :results none silent :exports none
  filename <- paste("../graphs/", basename, ".png", sep = "")
...
#+CAPTION:  Price vs. position in the simple model
#+NAME:     fig:CART2ModelBoston
#+ATTR_LaTeX: placement: [H]
[[../graphs/CART2ModelBoston.png]]
